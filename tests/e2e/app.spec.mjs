...
async function useGuest(page) {
  await page.addInitScript(() => localStorage.setItem('n2_guest_mode_v1', '1'));
}

test('first visit offers Google login and guest access', async ({ page }) => {
  await page.goto('/');
  ...
  await page.getByRole('button', { name: 'Dùng thử không đăng nhập' }).click();
