...
test('pet motion assets are versioned consistently to bypass a stale service-worker cache', () => {
  ...
  assert.match(index, /href="css\/styles\.css\?v=9"/);
  assert.match(index, /src="js\/app\.js\?v=9"/);
  assert.match(app, /from '\.\/pet\.js\?v=9'/);
  assert.match(worker, /n2-journal-v9/);
  assert.match(worker, /'\.\/css\/styles\.css\?v=9'/);
  assert.match(worker, /'\.\/js\/app\.js\?v=9'/);
  assert.match(worker, /'\.\/js\/pet\.js\?v=9'/);
  assert.match(worker, /'\.\/vendor\/pet-scene\.js\?v=9'/);
